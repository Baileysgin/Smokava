...
const EditRestaurant = () => {
  ...
  const loadRestaurant = async () => {
    ...
    try {
      ...
      message.error('خطا در بارگذاری اطلاعات رستوران');
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      message.success('رستوران با موفقیت به‌روزرسانی شد');
      navigate('/restaurants');
    } catch (error: any) {
      message.error(error.response?.data?.message || 'خطا در به‌روزرسانی رستوران');
    } finally {
      setLoading(false);
    }
  };
          ...
          >
            <Space direction="vertical" style={{ width: '100%' }}>
              <Upload
                ...
                accept="image/*"
              >
                <Button icon={<UploadOutlined />}>انتخاب تصویر جدید</Button>
              </Upload>
              {imagePreview && (
              ...
              <div style={{ marginTop: 8, fontSize: 12, color: '#999' }}>
                یا لینک تصویر جدید را وارد کنید:
              </div>
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value && !imageFile) {
                    ...
                    setImagePreview(restaurant?.image || restaurant?.imageUrl || null);
                  }
                }}
              />
            </Space>
